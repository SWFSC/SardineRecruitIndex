...
yoySard <- read_csv(file = paste0(datPath, "cciea_RREAS_YOYsardine_dl20250116.csv"), na = "NULL", skip = 1)
yoySard <- yoySard %>% rename(time = UTC,
                              sardYoY = `Relative CPUE (dglm model)`)
yoySard %>% ggplot(aes(x = time, y = sardYoY)) +
  geom_line()
```
