...
Documentation of data processing for Dynamic Factor Analysis of sardine recruitment indicators. Data are updated to cover through 2023 or later. Indicators directly relevant to anchovy have been removed.
...
Data were downloaded from the [CalCurrent IEA website](https://www.integratedecosystemassessment.noaa.gov/regions/california-current/california-current-iea-indicators) 1/16/2025. Monthly or seasonal values were aggregated across relevant time periods for fish life histories to create annual values for the DFA.
```{r}
# get data files to read in and collate

# data file path
datPath <- "C:/Users/r.wildermuth/Documents/CEFI/SardineRecruitmentESP/DFA_data/"
...
# use updated time series
datFiles <- datFiles[which(!grepl("2023", datFiles))]

# df names
datNames <- sub("_dl20250116.csv", "", datFiles)
datNames <- sub("_dl20250325.csv", "", datNames)
...
datDict <- data.frame(Variable = datNames,
                      Description = c(
                                      ...
                                      "Habitat compression index 30N to 35.5N",
                                      ...
                                      "Young-of-the-year sardine in SWFSC Rockfish Recruitment & Ecosystem Assessment Survey",
...
recrDat$OC_HCI_30N355N <- recrDat$OC_HCI_30N355N %>% filter(month %in% 2:7) %>%
                              group_by(year) %>%
                              summarize(HCI_30N355N = mean(OC_HCI_30N355N)) # Region 3
...
# # Read updated NRT dataset and aggregate to monthly time series similar to Mer
# # Downloaded from Mike Jacox (2/12/2025)
# WCNRT <- read_stars(paste0(datPath, "wcnrt_sst_daily_20110102_20240630.nc"), 
#                     sub = 3, curvilinear = c("lon_rho", "lat_rho"), proxy = FALSE)
# WCNRT <- st_transform(WCNRT, crs = st_crs("+proj=longlat +ellps=WGS84 +datum=WGS84"))
# 
# # Create bounding box for CalCOFI grid used by Mer (email from 3/24/2025)
# # Most northern, southern, western and eastern stations, respectively:
# # line/station 76.7,49,
# calCOFIstations <- matrix(c(35.0884327672,-120.77761231,
# # line/station 93.3,120,
#                             29.8460905906,-123.586475114,
# # line/station 93.3,120,
#                             29.8460905906,-123.586475114,
# # line/station 93.3,28,
#                             32.9087913558,-117.385498462, 
# # Diagonals
#                             35.0250407188,-120.926385792, 
#                              34.8935364583,-121.206645213,
#                              34.723639335,-121.553587787,
#                              34.3918126902,-122.251790429,
#                              34.0583872874,-122.942745523,
#                              33.7214985322,-123.635665298,
#                              32.8405611621,-117.526354339,
#                              32.6799104261,-117.872482381,
#                              32.5072812543,-118.207431003,
#                              32.3485483889,-118.557107667,
#                              32.1762365645,-118.889031952,
#                              32.0121154898,-119.234211524,
#                              31.8390928456,-119.571038136,
#                              31.5064951255,-120.246149557,
#                              31.1716012973,-120.917256938,
#                              30.8417498299,-121.586797921,
#                              30.5142659565,-122.251707266,
#                              30.1820846918,-122.923195089),
#                           ncol = 2, byrow = TRUE)
# calCOFIstations <- as.data.frame(calCOFIstations) %>% rename("lat" = "V1",
#                                                              "lon" = "V2")
# 
# calCOFIstations <- calCOFIstations %>% st_as_sf(coords = c("lon", "lat")) %>%
#                       summarize(geometry = st_union(geometry)) %>%
#                       st_convex_hull()
# st_crs(calCOFIstations) <- "+proj=longlat +ellps=WGS84 +datum=WGS84"
# 
# WCNRTcalCOFI <- st_crop(WCNRT, calCOFIstations)
# 
# test1 <-  aggregate(WCNRTcalCOFI, by = "months", FUN = mean)
...
# NRT transport downloaded from Mike Jacox (3/24/2025)
transpNRTOffshore <- read_ncdf(paste0(datPath, "monthly_northward_velocity_wcnrt_0-50m_124.0-120.0W_31.9-32.1N_2011-2024.nc"))

offshoreTrans <- data.frame(year = transportOffshore$year,
                            month = transportOffshore$month,
                            offshTrans = transportOffshore$v)
offTransNRT <- data.frame(year = transpNRTOffshore$year,
                            month = transpNRTOffshore$month,
                            offshTrans = transpNRTOffshore$v)

transport <- bind_rows(offshoreTrans, offTransNRT) %>% 
                mutate(seas = case_when(month %in% 1:6 ~ "spring",
...
# NRT transport downloaded from Mike Jacox (3/24/2025)
transpNRTNearshore <- read_ncdf(paste0(datPath, "monthly_northward_velocity_wcnrt_0-50m_120.0-115.0W_31.9-32.1N_2011-2024.nc"))

nearshoreTrans <- data.frame(year = transportNearshore$year,
                            month = transportNearshore$month,
                            nearshTrans = transportNearshore$v)
nearTransNRT <- data.frame(year = transpNRTNearshore$year,
                            month = transpNRTNearshore$month,
                            nearshTrans = transpNRTNearshore$v)

transport <- bind_rows(nearshoreTrans, nearTransNRT) %>% 
                mutate(seas = case_when(month %in% 1:6 ~ "spring",
                                          month %in% 6:12 ~ "summer"),
                         season = case_when(month %in% 3:5 ~ "spring",
                                          month %in% 6:8 ~ "summer",
...
transport %>% pivot_longer(cols = -year) %>%
  ggplot(aes(x = year, y = value)) +
    geom_line() + 
    facet_wrap(~name)

# create proportional offshore:nearshore flow index
#!!Note: this isn't a continuous metric:
#  - switches to negative when nearshore flow has net positive (northward) direction
#  - also only works if offshore flow is negative (southward)
#  - Sqrt transform removes some years with high positive northward flow
transport <- transport %>% mutate(sprRelOffTrans = as.numeric(avgOffTransspring/(avgOffTransspring+avgNearTransspring)),
                                  sumRelOffTrans = as.numeric(avgOffTranssummer/(avgOffTranssummer+avgNearTranssummer)),
                                  sprRelOffTrans = sqrt(sprRelOffTrans), # deal with some of the skewness
                                  sumRelOffTrans = sqrt(sumRelOffTrans),
                                  sumRelOffTrans = case_when(is.nan(sumRelOffTrans) ~ 0,
                                                             TRUE ~ sumRelOffTrans))
transport %>% pivot_longer(cols = c(sprRelOffTrans, sumRelOffTrans)) %>%
  ggplot(aes(x = year, y = value)) +
    geom_line() + 
    facet_wrap(~name)
```
...
indxGAM %>% select(year, sardLarv, mesopelLarv) %>%
...
SLiDERS index of young-of-the-year sardine from sea lion scats received from Alex Curtis (10/8/2024)
```{r}
yoySLiDERS <- read_csv(file = paste0(datPath, "NYOY.qtr.csv"), na = "NULL")
yoySLiDERS <- yoySLiDERS %>% filter(ScientificName == "YOY Sardinops sagax")

yoySLiDERS %>% ggplot(aes(x = as.factor(altYear), y = xn)) + 
  geom_bar(stat = "identity") +
  facet_wrap(~Season)

yoySLiDERS %>% group_by(Season) %>% summarize(avgN = mean(ns),
                                              minN = min(ns),
                                              maxN = max(ns),
                                              meanYoY = mean(xn))
# summer (June-Aug) has highest number of detections and second highest sample coverage
yoySLiDERS %>% filter(Season == "2-Su") %>%
  ggplot(aes(x = altYear, y = xn)) +
  geom_line()
```

Common thresher shark stomach contents from Antonella Preti (6/12/2025)
```{r}
thresher <- readxl::read_xlsx(path = paste0(datPath, "SWORDFISH_CCTD_FINAL_robert.xlsx"),
                              sheet = "Predator_Information")
shkCollections <- readxl::read_xlsx(path = paste0(datPath, "SWORDFISH_CCTD_FINAL_robert.xlsx"),
                              sheet = "Collection_Information")
preyComp <- readxl::read_xlsx(path = paste0(datPath, "SWORDFISH_CCTD_FINAL_robert.xlsx"),
                              sheet = "Prey_Composition")
preyComp <- preyComp %>% filter(Scientific_Name == "Sardinops sagax") %>% 
              select(Predator_ID, Prey_ID, Scientific_Name, Prey_N)

thresher <- thresher %>% left_join(y = shkCollections, by = "Collection_ID") %>% 
              select(Collection_ID, Predator_ID, Predator_Species, Month, Year, Region) %>%
              left_join(y = preyComp, by = "Predator_ID") %>%
              mutate(Prey_N = case_when(is.na(Prey_N) ~ 0,
                                        TRUE ~ Prey_N),
                     Month = as.numeric(Month),
                     Year = as.numeric(Year))

prevThresher <- readxl::read_xlsx(path = paste0(datPath, "Diet_Data_Preti_110320_FINAL_SUBMITTED.xlsx"),
                              sheet = "Predator_Information")
prevCollections <- readxl::read_xlsx(path = paste0(datPath, "Diet_Data_Preti_110320_FINAL_SUBMITTED.xlsx"),
                              sheet = "Collection_Information")
prevPreyComp <- readxl::read_xlsx(path = paste0(datPath, "Diet_Data_Preti_110320_FINAL_SUBMITTED.xlsx"),
                              sheet = "Prey_Composition")
prevPreyComp <- prevPreyComp %>% filter(Scientific_Name == "Sardinops sagax") %>% 
                  select(Predator_ID, Prey_ID, Scientific_Name, Prey_N)

prevThresher <- prevThresher %>% filter(Predator_Species == "Common Thresher Shark") %>%
                  left_join(y = prevCollections, by = "Collection_ID") %>% 
                  select(Collection_ID, Predator_ID, Predator_Species, Month, Year, Region) %>%
                  left_join(y = prevPreyComp, by = "Predator_ID") %>%
                  mutate(Prey_N = case_when(is.na(Prey_N) ~ 0,
                                            TRUE ~ Prey_N))

thresher <- bind_rows(thresher, prevThresher) %>% filter(Region != "NorCal, OR, WA") %>%
              mutate(Year = case_when(Month == 1 ~ Year - 1,
                                      TRUE ~ Year)) %>%
              group_by(Year) %>% 
              summarize(totSard = sum(Prey_N),
                        nStomachs = n(),
                        meanSard = mean(Prey_N),
                        nZeros = sum(Prey_N==0)) %>%
              mutate(meanPosSard = totSard/(nStomachs - nZeros))

thresher %>% select(Year, totSard, meanSard, meanPosSard) %>% 
...
# Weight-at-age data
```{r}
# From Alex Jensen (3/25/2025): columns(cruise, ship, haul, species, and specimen) is a unique fish identifier
waa <- read_csv(file = paste0(datPath, "Indiv assess age+weight.csv"))

waa <- waa %>% select(final_date, species, scientificName, weightg, age) %>% 
          filter(scientificName  == "Sardinops sagax") %>%
          mutate(year = year(final_date), month = month(final_date),
                 ...
                 season = case_when(month %in% 3:5 ~ "spring",
                                          ...
                                          month %in% 9:11 ~ "fall",
                                          month %in% c(12,1,2) ~ "winter")) 
table(waa$year, waa$month)
# !!RW: seasonal shift in sampling through time may bias a season-based indicator
#       or limit availability of forcing data in future (no spring samples after 2017)

waa <- waa %>%
          group_by(year, seas, age, scientificName) %>%
...
waa %>% group_by(scientificName, year, seas) %>%
...
sardSSBwt <- waa %>% filter(scientificName == "Sardinops sagax") %>%
                full_join(y = matProp, by = c("age" = "Age")) %>% 
                mutate(meanSSBwt = avgWAA * PropMature) %>% 
                group_by(scientificName, year, seas) %>%
...
Weight-length residuals using data from Caitlin Allen Akselrud 6/2/2025
```{r}
atBiolDat <- read_csv(file= paste0(datPath, "raw_trawl_specimen_data.csv"))

# get estimated length intervals, assuming extra
lengths <- data.frame(lenSL = seq(from = min(atBiolDat$standardLength_mm, na.rm = TRUE), 
                                  to = max(atBiolDat$standardLength_mm, na.rm = TRUE), 
                                  by = 1))
# convert to total length mm 
lengths$lenTL <- atm::convert_length(scientificName = "Sardinops sagax",
                                     L.in = lengths$lenSL, 
                                     from = "SL", to = "TL")

lengths$weight_est <- atm::estimate_weight(scientificName = "Sardinops sagax", 
                                           season = 1, model.type = "GLM", 
                                           totalLength_mm = lengths$lenTL)

atBiolDat <- atBiolDat %>% left_join(y = lengths, by = c("standardLength_mm" = "lenSL")) %>%
                mutate(year = year(final_date),
                       wtResid = weightg - weight_est,
                       condK = weightg/(lenTL^3)) %>%
                filter(sex == "female", standardLength_mm >= 160) %>%
                group_by(year) %>%
                summarize(meanResid = mean(wtResid, na.rm = TRUE),
                          sdResid = sd(wtResid, na.rm = TRUE),
                          meanK = mean(condK, na.rm = TRUE))

atBiolDat %>% pivot_longer(cols = -year, names_to = "variable", values_to = "value") %>%
  ggplot(aes(x = year, y = value)) +
  geom_line() +
  facet_wrap(~variable, scales = "free_y")
```
...
Spring Calanus pacificus
```{r}
spCpacificus <- read_csv(paste0(datPath, "Spring copepod community for Andrew 20221116.csv"))
spCpacificus <- spCpacificus %>% filter(Species_new == "Calanus pacificus")

spCpacificus %>% 
  filter(Year >= 1990) %>%
  ggplot(aes(x = Year, y = Abundance_m2)) +
  geom_line()
```
...
sardFiles <- expand.grid("sard", 3:7, 1998:2023, "mboost_roms.nc") %>% 
...
spawnTS <- data.frame(year = 1998:2023,
                      sardSpawnHab = t(sardSpawnHab$sard_3_1998_mboost_roms.nc))

spawnTS %>% pivot_longer(cols = c("sardSpawnHab")) %>%
  ggplot() +
...
```{r}
# aggregate over time
by_t <- as.Date(c("1998-01-01", "2023-01-01"))
...
```{r}
# sardine spawning season
sardFiles <- expand.grid("sardLarvae", 3:7, 1998:2023, "mboost_roms.nc") %>% 
                    mutate(sdmFile = paste(Var1, Var2, Var3, Var4, sep = "_")) %>%
                    pull(sdmFile)
...
nurseTS <- data.frame(year = 1998:2023,
                      sardNurseHab = t(sardLarvHab$sardLarvae_3_1998_mboost_roms.nc))

nurseTS %>% pivot_longer(cols = c("sardNurseHab")) %>%
  ggplot() +
  geom_line(aes(x = year, y = value, col = name))
```
...
sardVals <- SS_output("C:/Users/r.wildermuth/Documents/CEFI/SardineRecruitmentESP/SardineRecruitIndex/scenarioModels/2025_research_assessment_LorenzM",
...
yoySLiDERS <- yoySLiDERS %>% filter(Season == "2-Su") %>%
                rename(year = altYear,
                       yoySardSL = xn) %>%
                select(year, yoySardSL)

thresher <- thresher %>% select(Year, meanPosSard) %>%
              rename(year = Year,
                     posCThSk = meanPosSard)
...
spCpacificus <- spCpacificus %>% rename(year = Year,
                                        C.pacificus = Abundance_m2) %>%
                  select(year, C.pacificus)

# Join SS output and change name of 'Yr' once
outSS3 <-  sardRec  %>% rename(year = Yr) %>% 
...
recrDat <- recrDat %>% full_join(indxGAM, by = "year") %>%
              ...
              full_join(spCpacificus, by = "year") %>%
              ...
              full_join(yoySLiDERS, by = "year") %>%
              full_join(thresher, by = "year") %>%
              full_join(atBiolDat, by = "year") %>%
              select(-c(sprCalCOFILarvalSardine,
                        sprCalCOFISouthernMesopels,
                        sardGAMcoef, mesopelGAMcoef))

#write_csv(recrDat, file = "C:/Users/r.wildermuth/Documents/CEFI/SardineRecruitmentESP/SardineRecruitIndex/Data/recrDFAdat.csv")
```